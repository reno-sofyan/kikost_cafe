...
# Hook off-site opsional (mis. rclone). Tidak fatal bila gagal.
if [ -n "${BACKUP_OFFSITE_CMD:-}" ]; then
  echo "[backup] menjalankan hook off-site"
  sh -c "${BACKUP_OFFSITE_CMD}" "${OUT}" || echo "[backup] hook off-site GAGAL (lanjut)"
fi
